...
namespace FileStorage
{
    ...
    {
        static readonly IList<MimeTypeMapper> mappings = new List<MimeTypeMapper>
        {
        ...
        public static string GetExtension(this byte[] data)
        {
            foreach (var mapping in mappings)
            {
                if (mapping.Pattern.IsMatch(data))
        ...
        public static IReadOnlyCollection<string> SupportedTypes
        {
            get { return mappings.Select(x => x.Name).Distinct((StringComparer.OrdinalIgnoreCase)).ToList().AsReadOnly(); }
        }
